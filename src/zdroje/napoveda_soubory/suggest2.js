...
JAK.Suggest.prototype.action = function() {
	if (this._activeItem) { this.dom.input.value = this._activeItem.getValue(); }
	this._hide();
...
JAK.Suggest.prototype._clear = function() {
	...
	for (var i=0;i<this.items.length;i++) { this.items[i].$destructor(); }
	for (var i=0;i<this.remotes.length;i++) { this.remotes[i].$destructor(); }
	this.items = [];
	this.remotes = [];
	this._activeItem = false;
}
...
JAK.Suggest.prototype._buildUrl = function(query) {
	...
	arr.push("result=xml");
	if (this.options.prefix) { arr.push("prefix=1"); }
	if (this.options.highlight) { arr.push("highlight=1"); }
	if (this.options.count) { arr.push("count="+this.options.count); }
...
JAK.Suggest.prototype._keyNavigate = function(e, elm) {
	...
	if (code == 13) {
		if (this._used == 1 && this._activeItem) { this._used = 2; } /* odesilame po probehle navigaci a mame aktivni prvek */
		this.action();
		return;
	}
	...
	if (code == 27 || code == 9) { this._hide(); } /* esc */
}
...
JAK.Suggest.Item.prototype._action = function(e, elm) {
	this.owner._used = 2; /* klik mysi = plnohodnotne vyuziti suggestu */
	this.owner.action();
}

JAK.Suggest.Item.prototype._over = function() {
	if (this.owner._hoverLock) { return; }
